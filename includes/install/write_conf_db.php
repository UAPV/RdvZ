...
if (!is_writable('../') )
{
	...
	{$_SESSION['msg'].="<p>Erreur d'ecriture du fichier de configuration.</p>";
		...
		require_once('./connection_auth.php');
		$sql="select count(*) from ".$_POST["users_table"]." where ".$_POST["users_field"]."='rdvz' and ".$_POST["pwd_field"]."='rdvz_pwd'";
//		echo $sql."<br>";
		$res=$BDD_auth->getOne($sql);
				if ($res==0)
		{$sql="insert into ".$_POST["users_table"]."(".$_POST["users_field"].",".$_POST["pwd_field"].",". $_POST["email_field"].") values('rdvz','rdvz_pwd','".$_POST["email_admin"]."')";
		$res =$BDD_auth->query($sql);
		if (PEAR::isError($res)) {die($res->getDebugInfo());}}		   
		header('Location:success_db.html');
	}
}
?>